...
class MessageCreator:
    ...
    """
    def __init__(self, conn, reply_required=False, ack_required=False, ack_params=None):
        """
        constructor class
            Parameters:
    ...
    def createRebootMessage(self, host_name, package_filepath):
        """
        Create a HYPERVISOR_REBOOT message
        Parameters:
        ...
        return self.populateMessage("HYPERVISOR_REBOOT", {
            "host_name": host_name,
            "packages": pkgs
        })
    ...
    author="test", comment="testing Icinga API calls", is_flexible=False):
        """
        Create a HYPERVISOR_SCHEDULE_ICINGA_DOWNTIME message
        Parameters:
        ...
        if not host:
            print("host not found")
            return
        return self.populateMessage("HYPERVISOR_SCHEDULE_ICINGA_DOWNTIME", {
    ...
    def removeDowntimeMessage(self, host_name):
        """
        Create a HYPERVISOR_REMOVE_ICINGA_DOWNTIME message
        Parameters:
        ...
        if not self.conn.compute.find_hypervisor(host_name):
            print("host not found")
            return
        return self.populateMessage("HYPERVISOR_REMOVE_ICINGA_DOWNTIME", {
            "hypervisor":host_name
        })
    ...
    reason="vgc59244-admin: disabled for testing purposes"):
        """
        Create a HYPERVISOR_DISABLE_SERVICE message
        Parameters:
        ...
        if not service:
            ...
            return
        return self.populateMessage("HYPERVISOR_DISABLE_SERVICE", {
            "hypervisor":host_name,
    ...
    def enableServiceMessage(self, host_name, service_name="nova-compute"):
        ...
        Create a HYPERVISOR_ENABLE_SERVICE message
        Parameters:
        ...
        if not service:
            print("service not found on host")
            return
        return self.populateMessage("HYPERVISOR_ENABLE_SERVICE", {
            "hypervisor":host_name,
            "service_binary":service_name
        })
    ...
    def createVMMessage(self, server_name, image_name, flavor_name, network_name=None, zone_name=None, host_name=None):
        ...
        return self.populateMessage("CREATE_VM", {
            ...
            "hypervisor": (host_name if host else None),
            "zone": (zone_name if zone else None)
        })

    def getServerFromName(self, server_name):
        """
    ...
    def createServerStatusMessage(self, message_type, server_identifier, value):
        ...
        if server_identifier == "id":
            ...
            server_dict = self.getServerFromName(value)
        else:
        ...
        return {
            "SHUTDOWN": lambda message: self.populateMessage("SHUTDOWN_VM", message),
            "RESTART": lambda message: self.populateMessage("RESTART_VM", message),

            "PAUSE": lambda message: self.populateMessage("PAUSE_VM", message),
            "UNPAUSE": lambda message: self.populateMessage("UNPAUSE_VM", message),

            "SUSPEND": lambda message: self.populateMessage("SUSPEND_VM", message),
            "RESUME": lambda message: self.populateMessage("RESUME_VM", message),

            "REBOOT": lambda message: self.populateMessage("REBOOT_VM", message),
            "DELETE": lambda message: self.populateMessage("DELETE_VM", message),
        }.get(message_type.upper(), None)({"server": server_id})
